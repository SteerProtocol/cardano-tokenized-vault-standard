...
shorttitle: "CTVS-1"
subtitle: "Illustrated review edition"
...
companion: "CTVS-2, illustrated revision 0.6.3"
lang: en-US
---
...
Revision 0.6 keeps those exact object, role and recursive Data rules for reference profile 0. It separately specifies native-format execution conformance and a minimum common integration contract. Economic supply and verifiable ownership are common requirements; the ID/STATE token construction, single-state custody, virtual-balance formula and restricted-purpose transaction family are explicit reference choices. Native formats are not accepted on the strength of matching JSON alone.

\clearpage

# Semantic baseline revision 0.6: common guarantees, explicit implementations

This revision implements the agreed scope review of both v0.5 papers. It distinguishes **common economic and integration requirements** from the **fully specified reference construction**, defines a bounded route for native-format conformance, and adds a semantic direct-at-settlement delivery variant. It does not add a universal transcript language, module framework, permission language, routing engine, or hosted adapter runtime. [REV06]

| Identifier or decision | Revision 0.6 disposition |
|---|---|
| Document | Public-review revision 0.6, 11 September 2026; supersedes v0.5 for review |
| Existing reference wire | CTVS-WIRE-2; magic CTVS, integer 2, profile 0; unchanged |
| Reference schemas and fixtures | Exact files retained byte-for-byte from the v0.5 source bundle |
| Native integration | Explicit semantic mapping and evidence required; no native implementation certified here |
| Direct-at-settlement delivery | Specified semantic variant outside profile 0; no new wire tag or assigned profile integer |
| Budget fields | Existing storage, budget and exact-fee fields retained; no silent simplification |

**Compatibility.** This is a substantive clarification of conformance and an addition to the semantic design, not an editorial-only relabeling. It does not reinterpret profile 0, permit a different spend of its existing Requests, change its constructors, or change its terms-commitment bytes. A native implementation or a future reference-wire variant must bind its own reviewed version and execution rules. Never change a datum field or action under an old profile merely because a new PDF names a capability.

**Implementation release remains blocked.** Historical economic and codec results are carried forward, not rerun or promoted. This revision checks source/encoding preservation, mathematical examples, document cross-references, response examples and rendered publication layout. It adds no compiled validator, cryptographic witness check, real ledger transaction, measured execution limit or deployed protocol integration. Full historical counts and the shared REQ-01–REQ-14 register now live once in `docs/CTVS-Validation-Evidence.md` and `evidence/`, rather than being duplicated in both papers. [EVIDENCE]

The kickoff participants preferred retaining native contracts and datums where feasible. That is design input, not certification of any described protocol. New conformance and delivery language below is the CTVS proposal adopted for this revision; it must not be attributed to a verified SundaeSwap implementation. [WG, REV06]


\clearpage
# Illustration revision 0.6.3: figure index and reading key

This illustration-only refresh replaces the eleven remaining earlier-style figures. It retains the four approved images from the previous refresh and leaves the revision-0.6 semantic text, equations, exact reference encoding and commitment bytes unchanged. Illustration revision is not an on-chain wire version or a new validation result.

The replacement figures use a shared spacing grid, thin uniform outlines, restrained line icons and soft blue, violet, green and sand cards. Labels and captions identify the applicable profile. Colour supports grouping; it does not establish authority, success, or a new protocol rule. Arrows identify the relationship named in the figure, not the provenance of individual fungible coins. Dashed treatment marks the locally stated alternative or unsupported path.

Eleven replacements are vector SVG/PDF illustrations with selectable text; the four retained approved figures are image-based. The source package identifies each format and provides the actual artwork used in the build. Logical Mermaid sources are reference outlines, not an alternate production renderer.

| Figure | Placement | Primary question |
|---|---|---|
| 1. One standard boundary, two implementation paths | Section 2.5 | How can profile 0 and a native implementation satisfy the same guarantees? |
...
The diagrams explain the written requirements. They add no compiled-validator, ledger-acceptance, resource, managed-integration or independent-review evidence. The existing implementation release gates remain blocked.
...
**Scope rule.** CTVS standardizes authenticated economic meaning and enforceable user guarantees. It does not require one native order datum, strategy engine, permission mechanism or custody arrangement. Sections explicitly labeled reference profile 0 remain exact requirements of that profile. An alternative implementation must satisfy the common requirements through the mapping and evidence in Section 2.5; it does not inherit the reference profile's proofs by analogy.
...
The CTVS-WIRE-2 profile-0 combination, descriptively `DC-MB-IMM-RCI`, means direct custody, native mint/burn supply, immutable terms, and restricted composition inputs. It uses an enterprise state address, so there is no staking credential attached to that state. It does not assume or promise strategy yield. These are reference choices, not universal requirements for every future CTVS vault.
...
SundaeSwap’s published architecture supports script/multisig-owned orders and authorized scoopers. Minswap V2 specifies authorized batchers and pre-minted LP inventory. These examples motivate extensibility; they do not certify CTVS compatibility or current deployed bytecode. [10, 11]
...
CTVS-WIRE-2 remains the only exact **reference** wire in this publication. It supersedes WP1 sketches for reference profile 0, not the repository's V0 types or another protocol's native representation. Document revision 0.6, wire integer 2, profile integer 0, a native implementation version, deployed script hashes and evidence records are separate identities.

Profile 0 still fixes direct custody, mint/burn supply, immutable terms, the Section 6 reference fee/math rules, key roles, restricted composition and full-fill/snapshot/funded-claim asynchrony. Modes remain direct entry 1, direct exit 2, async entry 4 and async exit 8. `execution_modes` is a nonzero mask in 1..15; direct entry includes deposit and mint, direct exit includes withdraw and redeem. `max_batch` remains 1..16 as a provisional structural cap, not measured capacity. No direct-at-settlement, script-controller or managed-update action has been inserted into this encoding.

Native execution conformance below is not a second universal wire. It is a requirement for a specific implementation/version to expose and prove the common semantics using its own authenticated representation. Any future modification of profile 0's exact records requires an intentional new wire/profile decision and migration analysis. [WIRE, REV06]
...
| Common guarantee | Concrete reference-profile-0 choice |
|---|---|
...
Ordinary key inputs require their ledger authorization. Script inputs require successful validation of their spending conditions. Merely including a script as a reference input does not execute an authorization proof. Invalid-script collateral is separate from successful economic accounting; a failed transaction can affect collateral under the ledger’s collateral rules. [9]
...
The pinned Aiken transaction view is not the complete serialized transaction. Its fields include certificates, withdrawals, votes, proposals, current-treasury declarations, and donations. Omitting a field from a symbolic model does not prove it harmless. Profile 0 requires empty withdrawals, certificates, votes, and proposals, and absent current-treasury and donation fields. A zero-withdrawal aggregation hook is excluded too. Non-genesis minting is restricted to the configured SHARE delta. Unsupported purposes fail. [TX, PATTERNS]
...
The semantic transaction model uses symbolic ledger/signatory information. The wire-format checks exercise selected shape, role, and linkage predicates. Neither replaces phase-1 ledger checks, complete phase-2 execution, or cryptography. [SEMANTIC, WIRE]

# 4. Identity, deployment, and state succession

**Reference construction.** Section 4 specifies profile 0's exact authentication and deployment mechanism, not a factory every incumbent must redeploy. Every implementation must establish authentic assets, terms, accounting authority, succession and user-property separation. Section 2.5 requires a native proof of equivalent obligations; it does not accept look-alike metadata in place of the construction below.
...
OpenZeppelin documents virtual-balance and rounding defenses for vault share conversion. CTVS adopts the repository’s named arithmetic profile, while authenticating custody through UTxO transitions rather than external account balances. [12, R3]
...
A last-true binary search over the entire acceptance predicate can incorrectly return zero. The corrected algorithm searches only the monotonic **upper-resource predicate**: backing cap, supply bound, fee-liability bound, and permitted model value bounds. It then checks positive issuance at the upper candidate. [SEMANTIC, SEMANTIC-EVID]
...
Profile 0 chooses the simpler disjoint-output mechanism below. Relaxing its purpose restrictions without another demonstrated coverage proof is not an allowed editorial change.
...
The restricted family closes the reference proof's script-protected obligation set; it intentionally limits atomic composition. A shared allocation verifier or reviewed adapter is required before relaxing it. Request creation from another protocol and delivery to a script destination remain distinct from consuming that protocol's script during settlement. [WIRE, TX]
...
Distinct claims/refunds retain disjoint allocations. The exact ledger address subset and optional-datum representation are given in Section 10. Recipient data cannot be overwritten by a CTVS receipt; the separate claim object carries the source references. [WIRE]
...
Examples 2 and 4 advance the same sequence for redemption and fee collection. Their inner State action tags are respectively 3 and 5; no free-standing checkpoint field is added to the successor. Example 3 uses the same ten-field State structure with ADA represented by Terms.Asset constructor 0. These are candidate construction sketches, not serialized accepted transactions. [WIRE, WIRE-DATA]

# 10. Datum, redeemer, and encoding specification

**Normative for the existing reference wire.** The exact constructors, counts, bounds, names and commitment bytes in Section 10 are preserved from v0.5. They are not a mandatory migration format for every native integration. Native mappings must publish equally precise decoding and authentication rules for their own implementation and satisfy Section 2.5. The four files under `candidate-wire/` remain byte-identical; the revision checker verifies that preservation.
...
The 22 local fixtures and 44 cross-language comparisons are not builtin-certified golden bytes. Compare the exact nested types with the pinned Aiken decoder, compare commitment bytes with `serialiseData`, and obtain independently expected semantic vectors before freezing the encoding. This revision does not create those E3/E4 results. CTVS-2 Section 12 supplies the Request, Recovery, RequestBody and Claim records using these same primitives. [WIRE-DATA, WIRE-EVID]
...
\caption{Managed accounting adds position, liability, valuation, and liquidity evidence. A signed report identifies an attestation; it does not prove fair value or immediate exit liquidity. This is an extension boundary, not profile-0 behavior.}
...
These properties supplement, not replace, state/supply induction and rounding proofs. The economic predicates and byte/link predicates are separate programs; no completed proof connects their composition to actual compiled validator acceptance. [WIRE, SEMANTIC]
...
Profile 0 resolves its exact template/build, genesis, ID/STATE/SHARE and Config as specified earlier. A native implementation resolves the objects and guards in its reviewed Section 2.5 mapping. Both must support share-to-vault lookup with verified network and implementation identity. Labels, ticker symbols, a registry listing, descriptor metadata or a self-reported conformance badge are discovery hints, not proof of custody or execution rights.

## 13.2 Required response fields and value states

The following common semantic contract applies regardless of transport. The publication's `integration/response.schema.json` makes the initial snapshot, quote, request and build-effect envelopes machine-checkable; cross-field and ledger-evidence rules below remain mandatory beyond JSON shape validation. Its identifier `CTVS-INTEGRATION-0.6` is an off-chain draft schema revision, not an on-chain wire version.

| Response | Minimum required content |
|---|---|
| Every response | Schema identifier; record kind; network identity; chain point; vault ID; implementation binding; claimed conformance; evidence references; verification disposition. |
...
`implementation` identifies either profile 0's exact reference binding or a native implementation's mapping ID, mapping version, mapping commitment and source references. Every independently decoded field must trace to that binding. The hash algorithm, domain and exact bytes covered by a mapping commitment must be documented by the mapping; merely hashing an arbitrary provider response is not authentication. No universal mapping registry or code runtime is introduced.

An independent builder must derive the same permitted effect from the public mapping, not call the first builder's opaque construction service. A separately implemented decoder/indexer must agree on native bytes, economic field meanings, each supported operation and real rollback. A schema-valid illustrative response is not that acceptance test. The included response fixtures exercise publication shape only; no native protocol is declared conformant by this revision.

# 14. Resource limits, operational behavior, and risks

Minimum ADA depends on the actual serialized output and protocol parameters. CIP-55 gives the Babbage-era expression $(160+\operatorname{byteLength}(\operatorname{serializedOutput}))\,\operatorname{coinsPerUTxOByte}$. Implementations must use the active ledger-era calculation and parameter snapshot, not the numerical reserves in this paper. [6]
...
Profile-0 restrictions are deliberate interoperability limits: no foreign script-input composition, no pointer/bootstrap destination, no datum-hash destination, no protected attached reference script, and no script controller. A later profile can revise them only with explicit semantics, versioning and evidence. No capability or managed support follows from naming an extension. [WIRE]
...
## 15.2 Recorded evidence and this revision's checks

Both papers refer to one shared evidence record in `docs/CTVS-Validation-Evidence.md`; the historical results are not new runs and are not additive across the papers. They cover bounded economic/semantic checks and separate candidate-codec/link checks, not a complete compiled validator. Tests and models share an author; two languages do not establish independent review. [EVIDENCE]

Revision 0.6 additionally checks byte-for-byte preservation of the four candidate-wire files; unchanged reference fee/conversion examples; the budget reparameterization and direct-delivery value substitution; required response examples and deliberately invalid schema fixtures; document references and rendered layout. Arithmetic/source/schema results are in `checks/revision-results.json`; render/preflight results are in `checks/pdf-qa.json`. These are publication/schema/arithmetic checks, not native-protocol, ledger, resource or security-certification evidence. No newly proposed native delivery or script-controller mapping is recorded as implemented.

Every compiled-validator and ledger gate remains blocked. The exact old result counts and recorded provenance are consolidated once rather than repeated here. Request/claim/direct effects still need a jointly enforced implementation, and authentic initial identities remain premises until compiled genesis is demonstrated.
...
The baseline recorded in the repository is Aiken v1.1.22, standard library v3.1.0 and Plutus V3. This identifies the intended baseline, not the latest tooling or a successful compilation. Pin the ledger implementation and network parameters as well. [SEMANTIC, TX]
...
The v0.5 source basis is retained. ERC-4626, ERC-7540 and CIP-57 were rechecked on 11 September 2026 for their interface context; no fresh contract or deployed-protocol audit was performed. New scope, mapping and delivery requirements are CTVS proposals, not claims of SundaeSwap conformance. Repository references R1–R3 are pinned to the reviewed revision. External protocol documentation describes published designs and is not a certification of deployed instances. The equations, proposed profiles, worked examples, and proof sketches in this paper are the proposed CTVS design unless explicitly attributed otherwise.

[R1] Steer Protocol. Cardano Tokenized Vault Standard, reviewed repository. <https://github.com/SteerProtocol/cardano-tokenized-vault-standard/tree/f5f782de9ba6a7bf6cbb8213995a31734888559c>

[R2] CTVS architecture-hardening review, 8 September 2026. User-provided planning artifact, `CTVS-architecture-and-validation-review.md`.

[R3] Steer Protocol. Proposed wire format and accounting kernel, `lib/ctvs/wire.ak` and `lib/ctvs/math.ak`, revision identified in R1.

[1] ERC-4626: Tokenized Vaults. <https://eips.ethereum.org/EIPS/eip-4626>

[2] ERC-7540: Asynchronous ERC-4626 Tokenized Vaults. <https://eips.ethereum.org/EIPS/eip-7540>

[3] Cardano CIP-31: Reference inputs. <https://cips.cardano.org/cip/CIP-0031>

[4] Cardano CIP-32: Inline datums. <https://cips.cardano.org/cip/CIP-0032>

[5] Cardano CIP-33: Reference scripts. <https://cips.cardano.org/cip/CIP-0033>
...
[8] Cardano CIP-19: Cardano Addresses. <https://cips.cardano.org/cip/CIP-0019>

[9] Cardano CIP-40: Collateral Output. <https://cips.cardano.org/cip/CIP-0040>

[10] SundaeSwap. Published contract architecture. <https://github.com/SundaeSwap-finance/sundae-contracts>

[11] Minswap. AMM V2 specification. <https://github.com/minswap/minswap-dex-v2/blob/main/amm-v2-docs/amm-v2-specs.md>

[12] OpenZeppelin. ERC-4626 documentation. <https://docs.openzeppelin.com/contracts/5.x/erc4626>

[13] Cardano CIP-30: Cardano dApp-Wallet Web Bridge. <https://cips.cardano.org/cip/CIP-0030>


[PRIOR1] CTVS-1, revision 0.3, supplied main whitepaper, 9 September 2026.

[PRIOR2] CTVS-2, revision 0.3, supplied main whitepaper, 9 September 2026.

[SEMANTIC] CTVS semantic transaction model and economic review. Scope, findings, and limitations are summarized in `docs/CTVS-Validation-Evidence.md`. Recorded baseline: 8 September 2026.

[SEMANTIC-EVID] CTVS semantic verification evidence. `evidence/semantic-results.json` and the accompanying validation-evidence summary. Bounded symbolic/economic results; not compiled-validator or ledger evidence.

[REGISTER] CTVS implementation assurance requirements, REQ-01 through REQ-14. `docs/CTVS-Validation-Evidence.md` and `evidence/assurance-register.json` retain a BLOCKED disposition for every implementation gate.

[WIRE] CTVS wire-format and eUTxO specification, candidate CTVS-WIRE-2. Recursive layouts and transaction obligations are integrated in CTVS-1 Section 10 and CTVS-2 Section 12; supporting evidence is summarized in `docs/CTVS-Validation-Evidence.md`.

[WIRE-DATA] CTVS-WIRE-2 candidate artifacts: `candidate-wire/schema.json`, `wire.cddl`, `golden.json`, and `types.ak`. Type declarations are uncompiled; candidate bytes have not been certified against the Plutus builtin.

[WIRE-EVID] CTVS wire-format verification evidence, `evidence/wire-results.json`. Local codec/schema and selected boundary checks; not a complete economic validator. Scope and counts are recorded in the accompanying evidence summary.

[CIP31] Cardano CIP-31, Reference Inputs. <https://cips.cardano.org/cip/CIP-0031>

[CIP32] Cardano CIP-32, Inline Datums. <https://cips.cardano.org/cip/CIP-0032>

[TX] Aiken standard library v3.1.0, cardano/transaction.ak. Recorded blob `12f8ec80fdfe6c9976d63f3bb701f4fd525d3ec3`. <https://github.com/aiken-lang/stdlib/blob/v3.1.0/lib/cardano/transaction.ak>

[ADDR] Aiken standard library v3.1.0, cardano/address.ak. Recorded blob `0167b90fbbd4ad063db2725c44023f8d0bab42d6`. <https://github.com/aiken-lang/stdlib/blob/v3.1.0/lib/cardano/address.ak>

[SER] Cardano CIP-42, serialiseData. <https://cips.cardano.org/cip/CIP-0042>

[PLUTUS] IntersectMBO Plutus, PlutusCore/Data.hs. Recorded source blob: `a994354fb3c87a55ed6d1b4b6b544c7cd94c1c60`. <https://github.com/IntersectMBO/plutus/blob/master/plutus-core/plutus-core/src/PlutusCore/Data.hs>

[MINADA] Cardano CIP-55, Protocol Parameters (Babbage Era). <https://cips.cardano.org/cip/CIP-0055>

[BLUEPRINT] Cardano CIP-57, Plutus Contract Blueprint. <https://cips.cardano.org/cip/CIP-0057>

[VALIDATOR] Aiken, Validators. <https://aiken-lang.org/language-tour/validators>

[PATTERNS] Aiken, Common Design Patterns. <https://aiken-lang.org/fundamentals/common-design-patterns>

\clearpage

[BASE05] CTVS-1 and CTVS-2 revision 0.5, 9 September 2026, and the accompanying public source bundle. Direct editorial/technical baseline for this revision.

[WG] Cardano Vault Standard Working Group Kickoff, supplied transcript. In particular the opening discussion of avoiding unnecessary contract changes; the lending share-rate discussion; and the native-order/datum and controller discussion. Meeting statements are design input, not verified deployed behavior.

[REV06] Agreed scope review in the project conversation and `docs/CTVS-v0.6-Change-Map.md`. New recommendations are incorporated as expressly scoped proposal changes.

[REV-CHECKS] Revision 0.6 publication/arithmetic/schema checks, `checks/verify_revision.py` and `checks/revision-results.json`. Not compiled-validator or ledger evidence.

[EVIDENCE] Shared `docs/CTVS-Validation-Evidence.md` and recorded `evidence/` artifacts. Carries historical results with their original limitations and records this revision's separate checks.

# Appendix A. Revision boundary and delivery checklist

| Decision from the scope review | Result in revision 0.6 |
|---|---|
| Preserve exact UTxO/datum hardening | Profile-0 reference construction and all candidate-wire files retained |
| Stop treating one construction as universal | Common guarantees separated; native-format conformance now explicit |
| Avoid new transcript/module/permission frameworks | None required by this revision |
...
| Avoid mandatory intermediate claim for every native flow | Bounded direct-at-settlement semantic variant added outside profile 0 |
...
The source bundle includes both revised Markdown papers, the unchanged reference candidate files, common response schema and examples, a revision change map, the shared evidence record, new publication checks and the PDF build tooling. `python build_pdfs.py` typesets PDFs; `python checks/verify_revision.py` runs the stated local publication/arithmetic/schema checks. Neither compiles a contract or accepts a ledger transaction. Final publication QA must inspect rendered equations, tables, headings, cross-references and links.

Document revision 0.6 does not assign an on-chain profile to a native mapping or to `delivery_at_settlement`. The reference remains CTVS-WIRE-2, wire integer 2, profile 0, with identical committed bytes. Conformance claims must name the actual implementation, supported operation and evidence; a document version alone certifies none of them.
